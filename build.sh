...
echo "=== X-Plane OpenAL Shared Example Plugin Build Script ==="
...
if [ -f "$XPL_FILE" ]; then
    ...
    echo "1. Create directory: X-Plane/Resources/plugins/OpenALSharedExample/"
    echo "2. Copy mac.xpl to that directory"
    echo "3. Copy sound.wav file to the same directory"
    echo "4. Restart X-Plane"
    echo ""
    echo "Plugin Features:"
    echo "- Demonstrates OpenAL sound playback in X-Plane"
    echo "- Plays sound when aircraft is loaded"
    echo "- Pitch increases with each playback"
else
